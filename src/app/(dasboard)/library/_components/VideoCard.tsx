...
export const VideoCard = (props: Video & { user: User }) => {
      ...
      return (<Link href={`/share/${id}`} className='overflow-hidden group cursor-pointer rounded-2xl transition-all duration-200 hover:ring-primary ring-transparent ring-2   border-border border'>
            <div className='h-[14rem] z-12  bg-[#F8F8F8] relative'>
                  <div onClick={(e) => { e.preventDefault(); e.stopPropagation(); }} className='absolute gap-2 hidden  z-12 p-3 top-0  right-0 h-full group-hover:flex flex-col '>
                  ...
                  <Image className='w-full h-full z-0 object-center' width={500} height={500} src={thumbnailUrl! as string} alt='Poster' loading="lazy" />
            </div>
